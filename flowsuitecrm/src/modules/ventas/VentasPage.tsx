...
export function VentasPage() {
  ...
  const loadVentas = useCallback(async () => {
    ...
    const { data, error: fetchError } = await supabase
      .from('ventas')
      .select(
        'id, numero_nota_pedido, cliente_id, vendedor_id, producto_id, tipo_movimiento, monto, fecha_venta, created_at'
      )
  ...
  const rows = useMemo<DataTableRow[]>(() => {
    return ventas.map((venta) => {
      const tipoLabel = venta.tipo_movimiento
        ? t(`ventas.tipo.${venta.tipo_movimiento}`)
        : '-'
      const clienteLabel = venta.cliente_id
        ? clienteMap.get(venta.cliente_id) ?? venta.cliente_id
        : '-'
      const productoLabel = venta.producto_id
        ? productoMap.get(venta.producto_id) ?? venta.producto_id
        : '-'
      const vendedorLabel = venta.vendedor_id
        ? usersById[venta.vendedor_id] ?? venta.vendedor_id
        : '-'
      return {
        id: venta.id,
        cells: [
        ...
        detail: [
          ...
          { label: t('ventas.fields.monto'), value: venta.monto ?? '-' },
          { label: t('ventas.fields.fechaVenta'), value: venta.fecha_venta ?? '-' },
        ],
      }
    })
  }, [clienteMap, numberFormat, productoMap, t, usersById, ventas])
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (ventaOwnerType === 'cliente') {
      ...
      if (!prospectoId) {
        setFormError(t('ventas.errors.selectProspecto'))
        setSubmitting(false)
        return
      }
      const cuenta = prospectoCuenta.trim()
      if (!cuenta) {
        setFormError(t('ventas.errors.accountRequired'))
        setSubmitting(false)
        return
      }
      const prospecto = leads.find((lead) => lead.id === prospectoId)
      if (!prospecto) {
        setFormError(t('ventas.errors.prospectoMissing'))
        setSubmitting(false)
        return
      }
      ...
      const { error: leadUpdateError } = await supabase
        .from('leads')
        .update({ estado_pipeline: 'cierre', next_action: 'Convertido' })
        .eq('id', prospectoId)
  ...
  const buildDireccionValue = () => {
    ...
    return Object.values(direccionPayload).some((value) => value)
  ...
  const handleCreateCliente = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault()
    if (!configured) {
      setClienteFormError(t('common.supabaseRequired'))
      return
    }
  ...
  const handleCreateProducto = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault()
    if (!configured) {
      setProductoFormError(t('common.supabaseRequired'))
      return
    }
  ...
  return (
    <div className="page-stack">
      <SectionHeader
        ...
        action={<Button onClick={handleOpenForm}>{t('common.newVenta')}</Button>}
      />
      {!configured && (
        <EmptyState
          ...
          {ventaOwnerType === 'cliente' ? (
            ...
            <>
              <label className="form-field">
                ...
                <select value={prospectoId} onChange={(event) => setProspectoId(event.target.value)}>
                  <option value="">{t('common.select')}</option>
                  {loadingOptions && <option value="">{t('common.loading')}</option>}
                  {leads.map((lead) => (
              ...
              <label className="form-field">
                ...
                <input value={prospectoCuenta} onChange={(event) => setProspectoCuenta(event.target.value)} />
              </label>
            </>
          )}
          <label className="form-field">
          ...
          <label className="form-field">
            ...
            {productos.length === 0 && (
              <button
                ...
                className="inline-link"
                onClick={() => {
                  setProductoFormValues(initialProductoForm)
                  setProductoFormError(null)
      ...
      <Modal
        ...
        actions={
          <>
            <Button variant="ghost" type="button" onClick={() => setClienteFormOpen(false)}>
              {t('common.cancel')}
            </Button>
      ...
      >
        <form id="cliente-quick-form" className="form-grid" onSubmit={handleCreateCliente}>
          <label className="form-field">
            ...
            <input value={clienteFormValues.nombre} onChange={handleClienteChange('nombre')} />
          </label>
          <label className="form-field">
            <span>{t('clientes.fields.apellido')}</span>
            <input value={clienteFormValues.apellido} onChange={handleClienteChange('apellido')} />
          </label>
          <label className="form-field">
            <span>{t('clientes.fields.email')}</span>
            <input type="email" value={clienteFormValues.email} onChange={handleClienteChange('email')} />
          </label>
          <label className="form-field">
            <span>{t('clientes.fields.telefono')}</span>
            <input value={clienteFormValues.telefono} onChange={handleClienteChange('telefono')} />
          </label>
          <label className="form-field">
            <span>{t('clientes.fields.direccion')}</span>
            <input value={clienteFormValues.direccion} onChange={handleClienteChange('direccion')} />
          </label>
          <label className="form-field">
            <span>{t('clientes.fields.apartamento')}</span>
            <input value={clienteFormValues.apartamento} onChange={handleClienteChange('apartamento')} />
          </label>
          <label className="form-field">
            <span>{t('clientes.fields.ciudad')}</span>
            <input value={clienteFormValues.ciudad} onChange={handleClienteChange('ciudad')} />
          </label>
          <label className="form-field">
            <span>{t('clientes.fields.estadoRegion')}</span>
            <input value={clienteFormValues.estado_region} onChange={handleClienteChange('estado_region')} />
          </label>
          <label className="form-field">
            <span>{t('clientes.fields.codigoPostal')}</span>
            <input value={clienteFormValues.codigo_postal} onChange={handleClienteChange('codigo_postal')} />
          </label>
          <label className="form-field">
            <span>{t('clientes.fields.numeroCuentaFinanciera')}</span>
            <input
              value={clienteFormValues.numero_cuenta_financiera}
              onChange={handleClienteChange('numero_cuenta_financiera')}
            />
          </label>
          <label className="form-field">
            ...
            <input
              type="number"
              value={clienteFormValues.saldo_actual}
              onChange={handleClienteChange('saldo_actual')}
            />
          </label>
          <label className="form-field">
          ...
          <label className="form-field">
            ...
            <input value={clienteFormValues.distribuidor_id} onChange={handleClienteChange('distribuidor_id')} />
          </label>
          <label className="form-field">
            <span>{t('clientes.fields.fechaNacimiento')}</span>
            <input
              type="date"
              value={clienteFormValues.fecha_nacimiento}
              onChange={handleClienteChange('fecha_nacimiento')}
            />
          </label>
          <label className="form-field checkbox-field">
            <span>{t('clientes.fields.activo')}</span>
            <input type="checkbox" checked={clienteFormValues.activo} onChange={handleClienteChange('activo')} />
          </label>
      ...
      <Modal
        ...
        actions={
          <>
            <Button variant="ghost" type="button" onClick={() => setProductoFormOpen(false)}>
              {t('common.cancel')}
            </Button>
      ...
      >
        <form id="producto-quick-form" className="form-grid" onSubmit={handleCreateProducto}>
          <label className="form-field">
            ...
            <input value={productoFormValues.codigo} onChange={handleProductoChange('codigo')} />
          </label>
          <label className="form-field">
            <span>{t('productos.fields.nombre')}</span>
            <input value={productoFormValues.nombre} onChange={handleProductoChange('nombre')} />
          </label>
          <label className="form-field">
            <span>{t('productos.fields.categoria')}</span>
            <input value={productoFormValues.categoria} onChange={handleProductoChange('categoria')} />
          </label>
          <label className="form-field">
            <span>{t('productos.fields.precio')}</span>
            <input type="number" value={productoFormValues.precio} onChange={handleProductoChange('precio')} />
          </label>
          <label className="form-field checkbox-field">
            <span>{t('productos.fields.activo')}</span>
            <input type="checkbox" checked={productoFormValues.activo} onChange={handleProductoChange('activo')} />
          </label>
