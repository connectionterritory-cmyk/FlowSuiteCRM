-- Reference: STATMENT HYCITE.pdf
-- Scope: statement snapshot/document model for DFP/revolving
-- Critical rule: monetary source of truth remains cob_financial_ledger + cob_revolving_accounts.
-- This proposal does NOT replace or mutate ledger truth.

begin;

-- ============================================================
-- 0146: cob_statements + cob_statement_lines
-- ============================================================

-- -----------------------------------------------------------------
-- 1) Statement header (snapshot/document)
-- -----------------------------------------------------------------
create table if not exists public.cob_statements (
  ...
  -- Core snapshot fields (visual/functional parity with Hy Cite statement)
  ...
  -- Operational message blocks
  mensaje_pago text,
  metodos_pago text,

  -- Document lifecycle / delivery
...
comment on table public.cob_statements is
  'Snapshot documental del estado de cuenta. NO es fuente de verdad monetaria. '
  'La verdad monetaria permanece en cob_financial_ledger + cob_revolving_accounts.';
...
-- -----------------------------------------------------------------
-- 2) Statement lines (period transactions shown to customer)
-- -----------------------------------------------------------------
create table if not exists public.cob_statement_lines (
  ...
  -- Visual line identity
  entry_type text,
  component_type text,
  description text not null,

  -- Amount shown in statement line
...
comment on table public.cob_statement_lines is
  'Líneas de transacción del estado de cuenta (período). Derivadas de ledger para visualización.';
...
-- enforce org consistency between statement header and statement lines
...
-- -----------------------------------------------------------------
-- 5) RLS (org_id scoped; no public access)
-- -----------------------------------------------------------------
...
-- Read-only access for authenticated users in their own org.
-- Writes must happen through controlled RPC (security definer).
...
-- -----------------------------------------------------------------
-- 6) Future RPC contract (documentation comments only)
-- -----------------------------------------------------------------
comment on table public.cob_statements is
