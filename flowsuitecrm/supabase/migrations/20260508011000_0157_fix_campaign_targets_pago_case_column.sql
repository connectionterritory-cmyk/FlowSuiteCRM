...
--
-- Source: RECONSTRUCTED from pg_get_functiondef() on remote Supabase.
-- Primary fix: pago.case_id → pago.cargo_vuelta_case_id in fn_get_cargo_vuelta_campaign_targets.
--   cob_pagos uses cargo_vuelta_case_id (not case_id); the old name caused the payment
--   exclusion filter to silently skip all rows, allowing re-contact of paid clients.
--
-- Additional changes in this version:
--   - estado filter: is distinct from 'Cerrado' → not in ('Cerrado','En Negociación','Acuerdo')
--   - provider matching: added 'resend' to production send detection
--   - cadence simplified: ultimo_intento/seguimiento_3_dias → ultimo_aviso_legal (attempt >= 3)
--
-- Objects affected:
--   - public.fn_get_cargo_vuelta_campaign_targets: full CREATE OR REPLACE
-- Verified in Supabase: function body matches pg_get_functiondef output.
