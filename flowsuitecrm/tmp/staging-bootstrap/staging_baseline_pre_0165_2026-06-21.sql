...
-- ============================================================
-- ROLLBACK TAIL: pre-0165 — elimina objetos de 0165 / 0166 / 0167
-- Generado: 2026-06-21
-- Propósito: dejar el staging en estado anterior a las migraciones
--            0165 / 0166 / 0167, que aún NO deben ejecutarse en staging.
--
-- IMPORTANTE:
--   - NO ejecutar contra producción.
--   - Este bloque solo debe aplicarse al nuevo staging (ahdefxyvfgjkwgkfkaxd).
--   - CASCADE en DROP TABLE elimina automáticamente: índices, triggers,
--     policies RLS y constraints de FK que referencian estas tablas.
-- ============================================================

-- ── 0167: tablas de auditoría de corridas documentales ───────────────────────
DROP TABLE IF EXISTS public.cob_document_generation_run_items CASCADE;
DROP TABLE IF EXISTS public.cob_document_generation_runs CASCADE;

-- ── 0166: función generadora de resúmenes CV ─────────────────────────────────
DROP FUNCTION IF EXISTS public.fn_cv_resumen_generar(uuid, date, date, date, uuid);

-- ── 0165: tablas de resúmenes y líneas (orden de dependencia hijo → padre) ───
DROP TABLE IF EXISTS public.cob_cv_resumen_lines CASCADE;
DROP TABLE IF EXISTS public.cob_cv_resumenes CASCADE;
DROP TABLE IF EXISTS public.cob_cv_balance_adjustments CASCADE;

-- ── 0165: índice adicional en cob_pagos ──────────────────────────────────────
DROP INDEX IF EXISTS public.cob_pagos_cv_case_fecha_balance_idx;

-- ── 0165: columnas nuevas en cob_pagos ───────────────────────────────────────
ALTER TABLE public.cob_pagos
  DROP COLUMN IF EXISTS monto_aplicado_balance,
  DROP COLUMN IF EXISTS fee_plataforma;

-- ── Fin del rollback tail pre-0165 ───────────────────────────────────────────
